...
/**
 * <p>
 * A connection provider that uses the Tomcat JDBC connection pool outside
 * Tomcat container
 * </p>
 *
 * <p>
 * To use this connection provider set:<br>
 * <code>hibernate.connection.provider_class&nbsp;org.hibernate.connection.TomcatJDBCConnectionProvider</code>
 * </p>
 *
 * <pre>
 ...
 * <br>
 * 
 * N.B.: All Tomcat JDBC connection pool properties are also supported by using
 * the hibernate.tomcatJdbcPool prefix.
 * 
...
public class TomcatJDBCConnectionProvider implements ConnectionProvider,
	...
	public void configure(Map props) throws HibernateException {
		try {
			...
			// Tomcat JDBC connection pool properties used to create
			// theDataSource
			...
			if ((isolationLevel != null)
				...
				tomcatJdbcPoolProperties.setDefaultTransactionIsolation(Integer
						.parseInt(isolationLevel));
			}

			// // Turn off autocommit (unless autocommit property is set)
			// Unfortunately since hibernate3 autocommit defaults to true but
			// usually you don't need if it, when working outside a
			// EJB-container
			...
			if ((poolSize != null) && (poolSize.trim().length() > 0)) {
				tomcatJdbcPoolProperties.setMaxActive(Integer
						.parseInt(poolSize));
			}
				...
				if (key.startsWith(PREFIX)) {
					...
					for (i = 0; i < methods.length; i++) {
						if (methods[i].getName().equalsIgnoreCase(
								"set" + property)) {
							Method m = methods[i];
							Object parameter = convertIntoTypedValue(
									m.getParameterTypes()[0], value);
							try {
								m.invoke(tomcatJdbcPoolProperties,
					...
					if (i >= methods.length) {
						log.error("Unable to parse property " + key
								+ " with value: " + value);
						throw new RuntimeException("Unable to parse property "
			...
			if (ds.getPoolProperties().isJmxEnabled()) {
				...
				try {
					objectname = new ObjectName("ConnectionPool:name="
							+ tomcatJdbcPoolProperties.getName());
					if (!mBeanServer.isRegistered(objectname)) {
						mBeanServer.registerMBean(ds.getPool().getJmxPool(),
								objectname);
	...
	private Object convertIntoTypedValue(Class clazz, String value) {
		if (clazz.isAssignableFrom(boolean.class)) {
			...
			throw new RuntimeException("Unsupported Parameter type " + clazz);
